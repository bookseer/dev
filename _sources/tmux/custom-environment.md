...
Ключом к этому является опция `-t` или &laquo;target&raquo;.
...
Создадим новый сеанс tmux под названием &laquo;development&raquo;:
```console
...
В разделе {ref}`&laquo;Изучение основ&raquo; <tmux_basic>` были описаны команды tmux `new-session` и `new-window`.
...
Во-вторых, назначим окну имя &laquo;editor&raquo;.
...
Так &laquo;заставит&raquo; tmux нажать клавишу {bdg-dark}`Enter` после ввода команды.
...
настроена до конца.
Давайте разделим главное окно пополам по горизонтали &laquo;editor&raquo;, чтобы у нас было небольшое окно терминала внизу.
...
В файле конфигурации, который был создан в разделе {ref}`&laquo;Настройка tmux&raquo; <tmux_configuring>`, была переопределена нумерация окон и панелей с единицы.
...
TODO Здесь будет изображение ./img/-060-018.png
:::
-->


Одним из недостатков такого подхода является то, что этот сценарий каждый раз создает новый сеанс.
Это создаст проблемы, если запустить его во второй раз, когда сеанс &laquo;development&raquo; уже запущен.
