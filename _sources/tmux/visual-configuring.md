...
По умолчанию строка состояния выглядит так выглядит так:
...
Такая настройкм будет обновлять строку состояния каждые 60 секунд.
Следует иметь в виду, что при запустке команд оболочки,
