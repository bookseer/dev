...
Это &laquo;сеанс&raquo; (session) tmux, и он работает так же, как обычный терминальный сеанс.
...
Если использовать tmux сколь-нибудь продолжительное время, лучше создать &laquo;именованные сеансы&raquo;.
...
Это очень просто.
Следующая команда создаст именованный сеанс под названием &laquo;base&raquo;:
```console
...
Запуск любой из этих команд приведет к запуску &laquo;именованного сеанса&raquo; tmux.
...
Как это сделать будет показано в разделе {ref}`&laquo;Настройка tmux&raquo;<tmux_configuring>`.
...
процессов внутри него, можно оставить все это работающим в фоновом режиме, &laquo;отсоединившись&raquo; (detaching) от сеанса.
...
Позже, когда возникнет необходимость, пользователь может &laquo;присоединиться&raquo; (attach) к сеансу и продолжить работу с того
...
<!--
В разделе &laquo;Переключение между сеансами&raquo; главы 6 будут рассмотрены другие способы перемещения между активными сеансами.
-->
...
Создадим именованный сеанс с именем &laquo;windows&raquo;:
```console
...
$ top
```
Первое окно имеет, заданное при его создании, имя &laquo;shell&raquo;, второе --- &laquo;top&raquo;.
...
Переименуем его в &laquo;processes&raquo;.
...
{ref}`&laquo;Настройка tmux&raquo;<tmux_configuring>`, будет показано как сделать так, чтобы нумерация окон начиналась с единицы,
а не с нуля.
...
Давайте создадим новый сеанс tmux под названием &laquo;panes&raquo;, чтобы можно было поэкспериментировать с тем, как работают
панели.
...
В {ref}`разделе &laquo;Настройка tmux&raquo; <tmux_configuring>` будут переопределены сочетания клавиш, чтобы упростить изменение
размеров панелей.
...
Про то, как использовать команды tmux из терминала будет рассказано в разделе {ref}`&laquo;Создание настраиваемых рабочих окружений в tmux&raquo; <tmux_custom-environment>`, а пока давайте изучим командный режим tmux, используя его для создания новых окон и панелей в рабочей области.
...
В разделе {ref}`&laquo;Настройка tmux&raquo; <tmux_configuring>`, будет показано, как создать несколько собственных сочетаний клавиш, чтобы сделать использование некоторых из этих команд еще проще.
...
:------------------------------------|:-----------------------------------------------------------------------------------
 ...
 `tmux new -s development`           | Создает новый сеанс под названием &laquo;development&raquo;.
 `tmux new -s development -n editor` | Создает сеанс с именем &laquo;development&raquo; и называет первое окно &laquo;editor&raquo;.
 `tmux attach -t development`        | Подключается к сеансу под названием &laquo;development&raquo;.
