...
В этом примере используется параметр `-g` &laquo;глобально&raquo;, который устанавливает параметр для всех создаваемых сеансов tmux.
...
На Mac переназначить клавишу {bdg-dark}`Caps Lock` можно в панели настроек клавиатуры в разделе &laquo;Системные настройки&raquo;. Просто нажмите кнопку &laquo;Клавиши-модификаторы&raquo; и измените действие для {bdg-dark}`Caps Lock` на &laquo;Control&raquo;.
...
В разделе {ref}`&laquo;Основы tmux&raquo; <tmux_basic>` было рассказано об окнах и о том, как при создании нескольких окон в сеансе, можно ссылаться на них по индексу. Индексация начинается с нуля, что немного неудобно, так как приходится использовать {bdg-dark}`PREFIX` {bdg-dark}`0` для доступа к первому окну.
...
Измените команду перезагрузки, чтобы отобразить текст &laquo;Reloaded!&raquo; при загрузке файла конфигурации:
```
# ~/.tmux.conf
...
Параметры `-v` и `-h` в `split-window` означают &laquo;вертикальное&raquo; и &laquo;горизонтальное&raquo; разделение, но для tmux вертикальное разделение означает создание новой панели под существующей панелью, чтобы панели укладываются вертикально друг на друга. Горизонтальное разделение означает создание новой панели рядом с существующей, чтобы панели располагались горизонтально на экране.

Итак, чтобы разделить окно по вертикали, используется &laquo;горизонтальное&raquo; разделение, а для разделения по горизонтали используется &laquo;вертикальное&raquo; разделение.
...
## Переопределенные сочетания клавиш

Команда             | Описание
:-------------------|:----------
`CTRL-a`            | Новый `PREFIX`.
`PREFIX` `a`        | Отправляет `CTRL-a` программе, работающей в окне или панели tmux.
`PREFIX` `r`        | Перезагружает файл конфигурации tmux.
`PREFIX` `\|`       | Разделяет окно по горизонтали.
`PREFIX` `-`        | Разделяет окно по вертикали.
`PREFIX` `h`, `PREFIX` `j`, `PREFIX` `k`, `PREFIX` `l`   | Перемещается между панелями.
`PREFIX` `H`, `PREFIX` `J`, `PREFIX` `K`, `PREFIX` `L`   | Изменяет размер текущей панели
`PREFIX` `CTRL-h`, `PREFIX` `CTRL-l` | Перемещается вперед и назад между окнами.
