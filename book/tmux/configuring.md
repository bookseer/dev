...
```
После перезагрузки файла конфигурации можнно отправить {bdg-dark}`CTRL`-{bdg-dark}`a` приложению, работающему в tmux, просто дважды нажав {bdg-dark}`CTRL`-{bdg-dark}`a`.
...
Свяжем горизонтальное разделение окна с {bdg-dark}`PREFIX` {bdg-dark}`|` и вертикальное разделение c {bdg-dark}`PREFIX` {bdg-dark}`-`.
